...
parser.add_argument(
    '--endpoint', required=True, default='http://localhost:8891/sparql', help='SPARQL endpoint URL')
parser.add_argument(
    '--port', default='1111', help='ISQL local port')
parser.add_argument(
    '--user', default='dba', required=True, help='ISQL user')
parser.add_argument(
    '--password', required=True, help='ISQL password')
parser.add_argument(
    '--dir', required=True, help='Output directory')
parser.add_argument(
    '--isql', required=True, help='Full path to ISQL executable')
...
for row in reader:
    ...
    try:
        ...
        call([ISQL_CMD, ISQL_PORT, ISQL_USER, ISQL_PASSWORD, "isql.script"])
